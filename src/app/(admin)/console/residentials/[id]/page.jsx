...
export default function ResidentialDetailPage({ params }) {
    ...
    useEffect(() => {
        console.log("📍 Page Mount - ID:", id);
        if (id && id !== "new") {
            console.log("🚀 Triggering fetches for ID:", id);
            fetchData();
            fetchAvisos();
        } else {
            console.log("ℹ️ New residential or no ID");
        }
    }, [id]);
    ...
    const fetchAvisos = async () => {
        console.log("📥 Starting fetchAvisos...");
        if (!id || id === 'new') return;
        try {
            ...
            console.log("✅ fetchAvisos success:", avisosResponse);
        ...
        try {
            ...
            if (id === "new") {
                const payload = {
                    ...
                    habitantes: parseInt(formData.habitantes || 0),
                };
    ...
    const handleConfirmDeleteNotice = async () => {
        ...
        try {
            await databases.deleteDocument(dbId, "avisos_comunidad", noticeToDelete);
            setAvisosList(prev => prev.filter(a => a.$id !== noticeToDelete));
                    ...
                    <div className="admin-surface rounded-xl shadow-sm border admin-border p-6 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="col-span-2">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-3">
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => handleChange({ target: { name: 'habitantes', value: parseInt(e.target.value) || 0 } })}
                                            min="0"
                                            className="w-full px-4 py-2 rounded-lg border admin-border bg-white dark:bg-gray-700 admin-text focus:ring-2 focus:ring-primary-500 outline-none"
                                        />
                                    </div>
                ...
                <AnimatePresence>
                    {showDeleteNoticeModal && (
                        ...
                        >
                            ...
                            >
                                <div className="flex justify-between items-center mb-4">
                                    <h3 className="text-lg font-bold admin-text">Eliminar Aviso</h3>
                                    <button onClick={() => setShowDeleteNoticeModal(false)} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                                        <X size={24} />
                                    </button>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col-reverse md:flex-row justify-end gap-3 mt-6">
                                        ...
                                        <button
                                            ...
                                            className="w-full md:w-auto bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                                        >
                                            {deletingNotice ? "Eliminando..." : "Eliminar"}
                                        </button>
                                    </div>
                                </div>
                            </motion.div>
